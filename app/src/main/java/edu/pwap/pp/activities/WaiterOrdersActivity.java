...
public class WaiterOrdersActivity extends AppCompatActivity
{
    ...
    @BindView(R.id.waiter_orders_linear_layout)
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.activity_waiter_orders);
        getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
        ButterKnife.bind(this);

        buttonRefreshOrderToDeliver = findViewById(R.id.buttonRefreshOrderToDeliverList);
    ...
    private void showActionsDialog(final int position)
    {
        ...
        builder.setItems(options, new DialogInterface.OnClickListener()
        {
            ...
            public void onClick(DialogInterface dialog, int which)
            {
                if (which == 0)
                {
                    deliverOrder(ordersList.get(position)).subscribe(new DisposableSingleObserver<Order>()
                    {
                        @Override
    ...
    private Single<Order> deliverOrder(Order order)
    {
        OrderApi api = RetrofitInitializer.getClient().create(OrderApi.class);
        OrderService orderService = new OrderService(new OrderRepository());
        Single<Order> single = orderService.deliverOrder(order.getId(), api);

        Toast.makeText(WaiterOrdersActivity.this,"Order " + order.getId()
                + " has been delivered ", Toast.LENGTH_SHORT).show();
